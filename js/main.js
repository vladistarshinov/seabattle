            ...
            for (let j = startCoordX; j < startCoordX + 3; j++) {
                ...
                for (let k = startCoordY; k < startCoordY + 3; k++) {
                    ...
                    if (j >= 0 && j < 10 && k >= 0 && k < 10) {
                        ...
                        // Исправить в checkCollision в if includes на has
                        this.collision.add(coord);

                        /* Если не использовать в game.collision коллекцию new Set()
...
const fire = (event) => {
    ...
    if (!game.shipCount || target.classList.length > 0 || target.tagName !== 'TD') return;
